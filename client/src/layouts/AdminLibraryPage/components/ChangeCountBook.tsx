...
function ChangeCountBook(arg: Props) {
  ...
  useEffect(() => {
    const fetchBookInState = () => {
      arg.book.copies ? setCount(arg.book.copies) : setCount(0);
      arg.book.copiesAvailable
        ? setRemaining(arg.book.copiesAvailable)
        : setRemaining(0);
    };
    fetchBookInState();
  }, []);
  ...
  async function inc() {
    const url = `${process.env.REACT_APP_API_URL}/admin/secure/increase/book/count/?bookId=${arg.book?.id}`;
  ...
  async function dec() {
    if (count == 0)
      return;
    const url = `${process.env.REACT_APP_API_URL}/admin/secure/decrease/book/count/?bookId=${arg.book?.id}`;
    const requestOptions = {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
  ...
  async function deleteBook() {
    const url = `${process.env.REACT_APP_API_URL}/admin/secure/delete/book/?bookId=${arg.book?.id}`;
    const requestOptions = {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
...
export default ChangeCountBook;